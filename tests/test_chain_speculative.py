"""Tests for M5 chain speculative decoding."""
...
def _run_spec_greedy(model_runner: ModelRunner, prompt: str, max_tokens: int) -> list[int]:
    seq = _make_seq(model_runner, seq_id=200, prompt=prompt)
    _prefill_first_token(model_runner, seq)

    runner = EagleRunner(
        EngineConfig(model_path=MODEL_PATH, enable_eagle=True, eagle_draft_len=4),
        model_runner,
    )
...
def test_run_chain_verify_logits_shape_and_invariant(model_runner: ModelRunner) -> None:
    seq = _make_seq(model_runner, seq_id=0, prompt="Hello")
    _prefill_first_token(model_runner, seq)

    old_cached = seq.num_cached_tokens
    old_outputs = list(seq.output_token_ids)
    draft_tokens = [seq.last_token_id] * 4

    logits, returned_old_cached, target_cached = model_runner.run_chain_verify(seq, draft_tokens)

    assert logits.shape == (1 + len(draft_tokens), model_runner.model.config.vocab_size)
    assert returned_old_cached == old_cached
    assert target_cached == old_cached + 1 + len(draft_tokens)
    assert seq.num_cached_tokens == old_cached
    assert seq.output_token_ids == old_outputs


def test_chain_spec_step_preserves_cache_invariant(model_runner: ModelRunner) -> None:
...
def test_chain_spec_matches_normal_greedy(model_runner: ModelRunner) -> None:
